...
Este modelo é suficiente para Websites, pois o cliente pede por páginas estáticas e o Servidor as fornece sempre que necessário. Mas essa arquitetura não permite um uso mais dinâmico de websites, algo como uma Aplicação Web. Sua estrutura é muito burocrática. Cada ação do Cliente precisa ser enviado ao Servidor, com um cabeçalho de todo o seu estado e mais uma série de dados sobre conexão, para ser processado e enviado de volta, também com um cabeçalho bastante carregado. O overhead que essa estrutura carrega impossibilita um envio contante de requisições para criar uma impressão de responsividade nativa maior. Algo inviável com essa limitação, por exemplo, é mudar pequenas coisas na página Web a cada ação do usuário. Não faz sentido um usuário alterar algo na página, como a entrada de um formulário, e o Servidor precisar renderizar toda a página de volta, com grande parte do arquivo HTML inalterado, somente para um feedback pequeno.

Da discussão gerada à partir desse problema, surgiu o Ajax (Asynchronous JavaScript e XML). Ajax é uma técnica de desenvolvimento Web usada para fazer requisições ao Servidor para receber dados por baixo dos panos, sem precisar recarregar a página inteira. Com ele, é possível receber dados novos e atualizá-los no código sem alterar o resto da página. Um exemplo dele é no Google (TODO link), no qual você começa a digitar uma busca e resultados já começam a aparecer mesmo antes de dar o comando Enter, sem a página recarregar. Nesse exemplo, ele requisita continuamente os resultados gerados pela busca, mesmo incompleta, enquanto ela é formada.

(TODO) A conexão só dura durante o processo de requisição entre ambos, ela é fechada uma vez que a requisição é satisfeita.
